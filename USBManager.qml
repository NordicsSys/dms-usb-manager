...
PluginComponent {
    ...
    IpcHandler {
        function open(): string {
            USBManagerService.refreshDevices();
            if (panelLoader.item)
                panelLoader.item.openPanel();
            return "USB_MANAGER_OPEN_SUCCESS";
        }

        function close(): string {
            if (panelLoader.item)
                panelLoader.item.closePanel();
            return "USB_MANAGER_CLOSE_SUCCESS";
        }

        function toggle(): string {
            if (panelLoader.item && panelLoader.item.visible)
                return close();
            return open();
        }
        ...
        onExited: exitCode => {
            if (exitCode !== 0) {
                ...
                if (root._monitorRetries > root._monitorMaxRetries) {
                    console.warn("USBManager: udisksctl monitor failed", root._monitorMaxRetries, "times, giving up. USB hotplug detection disabled.");
                    return;
                }
